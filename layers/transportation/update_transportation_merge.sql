...
CREATE MATERIALIZED VIEW osm_transportation_merge_linestring AS (
    ...
    FROM (
      ...
      WHERE (highway IN ('cycleway','motorway','trunk', 'primary') OR highway = 'construction' AND construction IN ('motorway','trunk', 'primary'))
...
CREATE MATERIALIZED VIEW osm_transportation_merge_linestring_gen5 AS (
    ...
    WHERE (highway IN ('cycleway','motorway','trunk') OR highway = 'construction' AND construction IN ('motorway','trunk'))
...
CREATE MATERIALIZED VIEW osm_transportation_merge_linestring_gen6 AS (
    ...
    WHERE (highway IN ('cycleway','motorway','trunk') OR highway = 'construction' AND construction IN ('motorway','trunk')) AND ST_Length(geometry) > 2000
) /* DELAY_MATERIALIZED_VIEW_CREATION */;
...
CREATE MATERIALIZED VIEW osm_transportation_merge_linestring_gen7 AS (
    ...
    WHERE (highway IN ('cycleway','motorway') OR highway = 'construction' AND construction = 'motorway') AND ST_Length(geometry) > 4000
) /* DELAY_MATERIALIZED_VIEW_CREATION */;
