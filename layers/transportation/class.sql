...
CREATE OR REPLACE FUNCTION highway_class(highway TEXT, public_transport TEXT, tags HSTORE = null) RETURNS TEXT AS $$
    SELECT CASE
        ...
        WHEN tags->'bicycle' IN ('yes', 'permissive', 'dismount') AND highway IN ('pedestrian', 'path', 'footway', 'steps', 'bridleway', 'corridor', 'track', 'residential', 'service') THEN 'cycleway'
        ...
        WHEN tags->'cycleway:both' IN ('lane', 'opposite_lane', 'opposite', 'share_bussway', 'shared', 'track', 'opposite_track') THEN 'cycleway'
...
CREATE OR REPLACE FUNCTION cycleway_subclass(highway TEXT, tags HSTORE = null) RETURNS TEXT AS $$
    SELECT CASE
        WHEN tags->'surface' IN ('unpaved', 'compacted', 'dirt', 'earth', 'fine_gravel', 'grass', 'grass_paver', 'gravel', 'gravel_turf', 'ground', 'ice', 'mud', 'pebblestone', 'salt', 'sand', 'snow', 'woodchips', 'ground;grass', 'grass;earth', 'grass;ground', 'gravel;ground', 'gravel;grass', 'asphalt;sand', 'dirt;grass', 'ground;gravel', 'grass;dirt', 'gravel;earth', 'paved;unpaved', 'unpaved;paved', 'grass;gravel') THEN 'unpaved'
        WHEN tags->'surface' IN ('paved', 'asphalt', 'cobblestone', 'concrete', 'concrete:lanes', 'concrete:plates', 'metal', 'paving_stones', 'sett', 'unhewn_cobblestone', 'wood', 'cement') THEN 'paved'
